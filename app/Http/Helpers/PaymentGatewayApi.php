...
class PaymentGatewayApi {
    ...
    use Paypal,Stripe,Manual,SslcommerzTrait,RazorTrait,FlutterwaveTrait,QrpayTrait;
    ...
    public function render() {
        ...
        if(!is_array($output)){
            $error = ['error'=>['Render failed! Please call with valid gateway/credentials']];
            return Helpers::error($error);
        }
    ...
    public function responseReceive($type = null) {
        ...
        if(empty($tempData) || empty($tempData['type'])){
            $error = ['error'=>['Transaction failed. Record didn\'t saved properly. Please try again.']];
            return Helpers::error($error);
        }
