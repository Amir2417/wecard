                            ...
                            <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif" id="modal-english" role="tabpanel" aria-labelledby="modal-english-tab">
                                ...
                                <div class="form-group">
                                    @include('admin.components.form.input',[
                                        ...
                                        'value'     => old($default_lang_code . "_item_title",$data->value->language->$default_lang_code->item_title ?? "")
                                    ])
                                </div>
                                <div class="form-group">
                                    @include('admin.components.form.input',[
                                        'label'     => "Heading*",
                                        'name'      => $default_lang_code . "_item_heading",
                                        'value'     => old($default_lang_code . "_item_heading",$data->value->language->$default_lang_code->item_heading ?? "")
                                    ])
                                </div>
                            </div>
                                ...
                                <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="modal-{{ $item->name }}" role="tabpanel" aria-labelledby="modal-{{$item->name}}-tab">
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            ...
                                            'value'     => old($lang_code . "_item_heading",$data->value->language->$lang_code->item_heading ?? "")
                                        ])
                                    </div>
                                </div>
                            @endforeach
